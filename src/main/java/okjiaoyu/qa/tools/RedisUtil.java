...
import okjiaoyu.qa.persistance.controller.EntityEnvController;
...
public class RedisUtil {
     ...
     */
    public String getStringFromRedis(String key, int env_id) {
        String result = null;
        Jedis jedis = null;
        try {
            //add code by zxz
            ...
            EntityEnv entityEnv = new EntityEnvController().selectByPrimaryKey(env_id);
            String redis_ip = entityEnv.getRedisIp();
            int redis_port = Integer.parseInt(entityEnv.getRedisPort());
            jedisPool = new JedisPool(jedisPoolConfig, redis_ip, redis_port);
            // end
    ...
    public String getStringFromRedis_V2(String key, String redis_ip, int redis_port) {
        String result = null;
        Jedis jedis = null;
        try {
            //add code by zxz
    ...
    public String getString(String key) {
        ...
        try {
            ...
            String redis_ip = RunningParameter.redisIp;
            int redis_port = RunningParameter.redisPort;
            jedisPool = new JedisPool(jedisPoolConfig, redis_ip, redis_port);
            // end
     ...
     * @author sumory.wu @date 2014年8月14日 下午5:59:09
     */
    public Object getThException(String key) {
        if (StringUtils.isBlank(key)) {
            return null;
        }
        Object result = null;
     ...
     * @author sumory.wu @date 2014年8月14日 下午6:01:19
     */
    public void setThrowException(String key, Object value) {
        Jedis jedis = null;
        try {
            jedis = ( Jedis ) jedisPool.getResource();
            jedis.set(getKey(key), object2Bytes(value));
...
//     * @return
//     */
//    public long setPermission(String key, List value, int during) {
//        this.set(key, value);
//        return this.expire(key, during);//第二个参数是秒数
//    }
//
...
//    }
    ...
    public synchronized long getAndIncrLongValue(String key, int increment) {
        long result = -1;
        Jedis jedis = null;
        try {
            jedis = ( Jedis ) jedisPool.getResource();
            ...
            String toStore = (stored + increment) + "";
            jedis.set(key, toStore);
            result = stored;
        } catch (JedisConnectionException e) {
            logger.error("Jedis Connection error", e);
            if (jedis != null) {
     ...
     * @param expireSeconds    lock expire time with specify seconds
     * @param waitMilliSeconds request wait time with specify milliseconds
     *                         <p>
     ...
     * @refer unLock
     */
    public void addLock(String key, int expireSeconds, int waitMilliSeconds) {

        if (StringUtils.isEmpty(key) || expireSeconds <= 0 || waitMilliSeconds <= 0) {
            throw new IllegalArgumentException();
        }

        int i = 0;
        while (simpleLock(key, expireSeconds) == false) {
            try {
                i++;
                Thread.sleep(waitMilliSeconds);
            } catch (InterruptedException e) {
//                logger.info("addLockException", e.toString());
                logger.info("addLockException");
            }

            if (i > 500) {
                throw new RuntimeException("addLock wait long time exception");
            }
        }
     ...
     * @return
     */
    private boolean simpleLock(String key, int expireSeconds) {
        Jedis jedis = null;
        try {
            jedis = ( Jedis ) jedisPool.getResource();
