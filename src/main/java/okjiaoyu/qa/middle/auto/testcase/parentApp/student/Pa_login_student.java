 ...
 */
public class Pa_login_student extends ParentApp_student {
...
//    }
    ...
    public void login() {
        ...
        requestSampler.setTestCaseName("登陆-学生版");
        //发送请求
        ...
        logger.info( "okay+学生版登录接口返回结果: " + responseJson );
        //account_token = responseJson.getJSONObject("data").get("token").toString();
        account_token=responseJson.getJSONObject( "data" ).getJSONObject( "childs" ).get( "token" ).toString();
        logger.info("okay+学生版登录返回account-token is: " + account_token);
        account_uid = responseJson.getJSONObject("data").getJSONObject( "childs" ).get("uid").toString();
        logger.info("okay+学生版登录返回account-uid is: " + account_uid);
    }
}
